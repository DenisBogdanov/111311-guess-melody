...
import failTimeOver from './fail-time-over';
...
const uiReplayButton = element.querySelector(`.game__back`);
uiReplayButton.addEventListener(`click`, (event) => {
  event.preventDefault();
  changeScreen(welcome);
});

const uiAnswers = [...element.querySelector(`.game__artist`).answer];

const uiGameOverScreens = [success, failLivesOver, failTimeOver];

uiAnswers.forEach((elem) => elem.addEventListener(`change`, () => {
  changeScreen(uiGameOverScreens[Math.floor(Math.random() * uiGameOverScreens.length)]);
}));
